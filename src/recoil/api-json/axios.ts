...
import { selectedApiSelector } from '..'

export const fetchQuerySelector = selector({
  key: 'fetchQuery',
  get: async ({ get }) => {
    const selectedApi = get(selectedApiSelector)
    const fetchRequest = await axios.get(selectedApi)
    console.log('fetchRequest', fetchRequest)
    return fetchRequest
  },
  set: ({ reset }) => {
    reset(axiosConfigAtom)
    reset(axiosDataAtom)
    reset(axiosHeadersAtom)
    reset(axiosRequestAtom)
    reset(axiosResponseAtom)
    reset(axiosStatusAtom)
    reset(axiosStatusTextAtom)
  }
})

// const [fetchQuery, setFetchQuery] = useRecoilState(fetchQuerySelector)
// const setFetchQuery = useSetRecoilState(fetchQuerySelector)
// const fetchQuery = useRecoilValue(fetchQuerySelector)
// const resetFetchQuery = useResetRecoilState(fetchQuerySelector)
// return a callback to clear cache
// const refreshFetchQuery = useRecoilRefresher_UNSTABLE(fetchQuerySelector)

// export const queryResponseSelector = selectorFamily({
//   key: 'queryResponse',
//   get: ({ get, set }) => {
//     const response = get(fetchQuerySelector)
//     set(axiosResponseAtom(await response))
//   },
//   set: ({ set, reset }) => {
//     set(axiosResponseAtom(await response))
